...
public:
  WavReader(const char *filename) {
    ...
    // LIUJUNNAN:ADD: WAVHEADER IS wrong, the header have some optianal parameters
    header.data_size = header.size - 36;
    int num_data = header.data_size / (bits_per_sample_ / 8);
    /* printf("size:%d, data_size:%d, bits_per_sample:%d \n", header.size, header.data_size, bits_per_sample_); */
